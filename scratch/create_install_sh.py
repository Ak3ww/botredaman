...
# ==============================================================================
# NOC Bot Redaman - Auto Installer for Ubuntu VPS (Oracle Free Tier)
# ==============================================================================
...
echo -e "${BLUE}=====================================================${NC}"
echo -e "${GREEN}🚀 NOC BOT REDAMAN - VPS AUTO INSTALLER 🚀${NC}"
echo -e "${BLUE}=====================================================${NC}"
...
echo -e "\\n${BLUE}[1/7] Mendapatkan IP Publik VPS...${NC}"
...
echo -e "\\n${BLUE}[2/7] Setup Konfigurasi Token & Chat ID...${NC}"
read -p "Masukkan Telegram Bot Token [8773632704:AAFschVyWAyGIwGyjU5mwt1xDlMs3I-NqGc]: " TELE_TOKEN
TELE_TOKEN=${TELE_TOKEN:-"8773632704:AAFschVyWAyGIwGyjU5mwt1xDlMs3I-NqGc"}

read -p "Masukkan Telegram Chat ID [298223450]: " TELE_CHAT_ID
TELE_CHAT_ID=${TELE_CHAT_ID:-"298223450"}

read -p "Masukkan Dashboard URL [http://${PUBLIC_IP}:8000]: " DASH_URL
DASH_URL=${DASH_URL:-"http://${PUBLIC_IP}:8000"}

read -p "Masukkan Interval Bulk Reminder (menit) [10]: " BULK_MIN
...
echo -e "\\n${BLUE}[3/7] Mengupdate sistem & menginstal Python, Pip, Virtualenv, SQLite3...${NC}"
sudo apt-get update -y
sudo apt-get install -y python3-pip python3-venv sqlite3 build-essential curl

# 4. Install Node.js v20
echo -e "\\n${BLUE}[4/7] Menginstal Node.js v20...${NC}"
...
# 5. Setup Virtual Environment Python & Install Requirements
echo -e "\\n${BLUE}[5/7] Setup virtualenv Python & install library...${NC}"
python3 -m venv venv
...
if [ -f "requirements.txt" ]; then
    ...
    echo -e "${RED}⚠ requirements.txt tidak ditemukan. Menginstal pysnmp & requests manual...${NC}"
    pip install pysnmp>=4.4.12 requests>=2.25.1
fi
deactivate

# Setup Dependensi Node.js
echo -e "\\n${BLUE}[5b/7] Menginstal dependensi Node.js...${NC}"
npm install

# 6. Konfigurasi Firewall Lokal (Unblock Port 8000)
echo -e "\\n${BLUE}[6/7] Membuka port 8000 di firewall iptables VPS...${NC}"
...
# 7. Daemom/Service Management (Systemd & PM2)
echo -e "\\n${BLUE}[7/7] Mendaftarkan layanan systemd & PM2...${NC}"
...
echo -e "\\n${GREEN}=====================================================${NC}"
echo -e "${GREEN}🎉 INSTALASI NOC BOT & DASHBOARD SELESAI 🎉${NC}"
echo -e "${GREEN}=====================================================${NC}"
echo -e "${YELLOW}Dashboard URL : ${DASH_URL}${NC}"
echo -e "${YELLOW}Status Services:${NC}"
echo -e "  - Web Dashboard (PM2) : ${GREEN}Running${NC} (Check with: pm2 status)"
echo -e "  - Collector (Systemd) : ${GREEN}Running${NC} (Check with: sudo systemctl status noc-collector)"
echo -e "  - Telegram Bot (System) : ${GREEN}Running${NC} (Check with: sudo systemctl status noc-telegram-bot)"
echo -e "${BLUE}=====================================================${NC}"
"""

# Save file with UNIX line endings LF
install_sh_path = r"C:\BotRedaman\deployment\install.sh"
with open(install_sh_path, "w", encoding="utf-8", newline="\n") as f:
    f.write(install_sh_content)

print("install.sh successfully written with UNIX LF line endings!")
